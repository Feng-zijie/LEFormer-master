...
if __name__ == '__main__':
    in_channels = 32
    height, width = 64, 64
    x = torch.rand(16, in_channels, height, width).cuda()
    edge_feature = make_laplace(x, channels=in_channels)
    block = EGA(in_channels).cuda()
    output = block(edge_feature, x)

    print("Edge feature size:", edge_feature.size())  # torch.Size([16, 32, 64, 64])
    print("Input feature size:", x.size())            # torch.Size([16, 32, 64, 64])
    print("Output size:", output.size())              # torch.Size([16, 32, 64, 64])
