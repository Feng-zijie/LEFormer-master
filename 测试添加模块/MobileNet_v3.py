...
class MobileNetV3_Small(nn.Module):
    ...
    def forward(self, x):
        _,c, h, w = x.size()
        out=nn.Conv2d(c, c, kernel_size=3, stride=1, padding=1, bias=False)(x)
        out=nn.BatchNorm2d(c)(out)
        out = self.hs1(out)

        if c==3:
            out=nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)(out)
            out = self.bneck1(out)
            out=nn.BatchNorm2d(32)(out)
            out = self.hs2(out)
            out=nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1, bias=False)(out)
            out=nn.BatchNorm2d(32)(out)

        elif c==32:
            out=nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=1, bias=False)(out)
            out = self.bneck2(out)
            out=nn.BatchNorm2d(64)(out)
            out = self.hs2(out)
            out=nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False)(out)
            out=nn.BatchNorm2d(64)(out)
        elif c==64:            
            out=nn.Conv2d(64, 16, kernel_size=3, stride=1, padding=1, bias=False)(out)
            out = self.bneck3(out)
            out=nn.BatchNorm2d(160)(out)
            out = self.hs2(out)
            out=nn.Conv2d(160, 160, kernel_size=3, stride=1, padding=1, bias=False)(out)
            out=nn.BatchNorm2d(160)(out)
        elif c==160:      
            out=nn.Conv2d(160, 16, kernel_size=3, stride=1, padding=1, bias=False)(out)      
            out = self.bneck4(out)
            out=nn.BatchNorm2d(192)(out)
            out = self.hs2(out)
            out=nn.Conv2d(192, 192, kernel_size=3, stride=1, padding=1, bias=False)(out)
            out=nn.BatchNorm2d(192)(out)
